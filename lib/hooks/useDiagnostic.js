...
/**
 * Which module each diagnostic area points back into. Deliberately NOT a
 * mapping into STUDY_PATHS — see CLAUDE.md's "Diagnostic Placement &
 * Concept Evidence": this diagnostic cannot reliably tell a CS major who
 * knows no quantum from a complete beginner, so it must not attempt to
 * auto-select a background-based Study Path. It only points at which of
 * the diagnostic-tested modules is worth starting with or skimming.
 *
 * Originally the 3 Phase 3a pilot areas (math-notation/states/gates,
 * mapped to braket/intuition/gates by area label, not by matching id).
 * Extended in TASK-039 with 4 more areas whose id is simply the same
 * string as the module id — no separate lookup needed for those, kept
 * here anyway for a single consistent source of truth.
 */
